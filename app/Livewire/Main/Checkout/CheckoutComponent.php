...
// ]);
    ...
    // dd($response);
                    ...
                    case 'ecpay':
                        ...
                        $PaymentType       = "aio";
                        $TotalAmount       = $amount;
                        $TradeDesc         = EcpayGateway::ecpayUrlEncode('易描述範例');
...
case 'stripe':
    try {
        // Set your secret key
        \Stripe\Stripe::setApiKey($selected->settings['secret_key']);
...
//   $this->webhook(['payment_id' => $session->client_secret, 'payment_method' => 'stripe']);
...
//                             ]
//                         );
//             //   dd($this->payment_gateway_params['stripe']);
//                         // Set intent
...
//          return redirect()->to('https://checkout.stripe.com/pay/' . $intent->client_secret);
//         //  dd($intent);
//         // Construct Stripe checkout URL
     
  
//     //  $stripeCheckoutUrl = 'https://checkout.stripe.com/pay/' . $intent->client_secret;
//     // return redirect()->to($stripeCheckoutUrl);
...
//     break;
    ...
    public function wallet()
    {
        try {
            ...
            auth()->user()->notify( (new OrderPlaced($order, $order->total_value))->locale(config('app.locale')) );
