                ...
                <div class="flex items-center lg:justify-center rtl:space-x-reverse space-x-2 mb-10">
                    ...
                    <span class="text-base font-bold text-slate-500 tracking-wide dark:text-zinc-300">
                        ...
                        <!--@lang('messages.t_enter_amount_u_like_to_deposit')-->
                         <!--@lang('messages.t_add_funds_subtitle')-->
                    </span>
                </div>
                ...
                <div class="w-full mb-6">
                    <div class="relative w-full">
                        ...
                        <div class="absolute inset-y-0 ltr:right-0 rtl:left-0 flex items-center ltr:pr-4 rtl:pl-4 font-light text-[15px] tracking-widest dark:text-gray-300 text-gray-400">
                            @if (isset( config('money.currencies.' . settings('currency')->code)['symbol'] ))
                                {{ config('money.currencies.' . settings('currency')->code)['symbol'] }}
                            @else
            ...
            <div class="lg:ltr:pl-10 lg:rtl:pr-10">
                ...
                <section class="bg-slate-100 p-8 rounded-lg dark:bg-zinc-800 mb-10">
                    ...
                    <dl class="mt-6 space-y-2">
                        ...
                        {{-- Exchange rate --}}
                        @if ( $payment_currency && is_array( config('money.currencies.' . $payment_currency) ) && !empty( $payment_gateway->exchange_rate ) && $payment_gateway->exchange_rate != settings('currency')->exchange_rate )
                ...
                @if ($selected_method === 'paymob' && isset($payment_gateway_params['paymob']))
                    <div class="w-full">
                        <iframe src="https://accept.paymobsolutions.com/api/acceptance/iframes/{{ payment_gateway('paymob')?->settings['iframe_id'] }}?payment_token={{ $payment_gateway_params['paymob']['token'] }}" width="100%" height="500px"></iframe>
                    </div>
                @endif
                ...
                @if ($selected_method === 'paymob-pk' && isset($payment_gateway_params['paymob-pk']))
                    <div class="w-full">
                        <iframe src="https://pakistan.paymob.com/api/acceptance/iframes/{{ payment_gateway('paymob')?->settings['iframe_id'] }}?payment_token={{ $payment_gateway_params['paymob']['token'] }}" width="100%" height="500px"></iframe>
                    </div>
                @endif
