    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 max-w-4xl mx-auto lg:divide-x lg:divide-gray-200 lg:dark:divide-zinc-700 rtl:divide-x-reverse space-y-10 lg:space-y-0" x-data="{ selected_method: '' }">
        ...
        <div class="lg:ltr:pl-10 lg:rtl:pr-10">
            ...
            <div class="w-full mb-8">
                ...
                {{-- Exchange rate --}}
                @if ($selected_method === 'nowpayments')
                    <div class="flex items-center space-x-3 rtl:space-x-reverse bg-gray-100 hover:bg-gray-200 hover:bg-opacity-50 py-4 px-5 rounded-xl dark:bg-zinc-800 mt-4">
                        <div class="grow">
                            <p class="text-sm text-gray-500 dark:text-zinc-400 font-light">
                        ...
                        <div class="flex-none text-sm ltr:text-right rtl:text-left font-semibold text-zinc-700 dark:text-zinc-200">
                            {{ number_format($total * $payment_gateway->exchange_rate / settings('currency')->exchange_rate) . ' ' . $payment_currency }}
                        </div>
                    </div>
                @elseif ($payment_currency && is_array( config('money.currencies.' . $payment_currency) ) && !empty( $payment_gateway->exchange_rate ) && $payment_gateway->exchange_rate != settings('currency')->exchange_rate)
                    <div class="flex items-center space-x-3 rtl:space-x-reverse bg-gray-100 hover:bg-gray-200 hover:bg-opacity-50 py-4 px-5 rounded-xl dark:bg-zinc-800 mt-4">
                        <div class="grow">
                            <p class="text-sm text-gray-500 dark:text-zinc-400 font-light">
                                @lang('messages.t_exchange_rate')
...
//                  console.log('Received checkout-stripe-order-event with client secret:', clientSecret);
//                 setTimeout(function () {
//                      console.log('Redirecting to Stripe checkout with client secret:', clientSecret);
// //   window.location.href = 'https://checkout.stripe.com/pay/' + clientSecret;
//                     // Stripe public key
//                     // console.log("Stripe public key:", "{{ payment_gateway('stripe')?->settings['public_key'] }}");
