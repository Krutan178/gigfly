<div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 mt-[7rem] py-12 lg:pt-16 lg:pb-24" x-data="window.oZcfXWmBuWfxbIo" x-init="initialize()">
                        ...
                        @if ($gig->childcategory)
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <a href="{{ url('categories/' . utf8_decode(urldecode($gig->category->slug . '/' . $gig->subcategory->slug . '/' . $gig->childcategory->slug))) }}" class="ltr:ml-2 rtl:mr-2 text-sm font-medium text-gray-400 hover:text-gray-600" aria-current="page">{{ $gig->childcategory->name }}</a>
                                </div>
                            </li>
                        @endif
    ...
    <li class="splide__slide dark:bg-zinc-700 bg-gray-200 object-contain">
        <img src="{{ Storage::disk('s3')->url('gigs/gallery/large/' . $image->large->file_name) }}" alt="{{ $gig->title }}" class="object-contain">
    </li>
@endforeach
            ...
            <div class="max-w-2xl mx-auto mt-6 md:mt-14 sm:mt-16 lg:max-w-none lg:mt-0 lg:row-end-2 lg:row-span-2 lg:col-span-3">
                ...
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
                    ...
                    {{-- Contact seller --}}
                    <a href="{{ url('messages/new', $gig->owner->username) }}" target="_blank" class="w-full bg-gray-100 border border-transparent rounded-md py-4 px-8 flex items-center justify-center text-[13px] font-medium text-gray-600 hover:bg-gray-200 focus:outline-none dark:bg-zinc-700 dark:hover:bg-zinc-600 dark:text-zinc-300 dark:hover:text-zinc-200">{{ __('messages.t_contact_seller') }}</a>
...
@push('scripts')
    ...
    {{-- Splide Plugin --}}
    <script defer src="{{ asset('js/plugins/splidejs/js/splide.min.js') }}"></script>
    <script defer src="{{ asset('js/plugins/splidejs/js/splide-extension-video.min.js') }}"></script>
...
@push('styles')
    ...
    {{-- Splide Plugin --}}
    <link rel="stylesheet" href="{{ asset('js/plugins/splidejs/css/splide.min.css') }}">
    <link rel="stylesheet" href="{{ asset('js/plugins/splidejs/css/splide-core.min.css') }}">
    <link rel="stylesheet" href="{{ asset('js/plugins/splidejs/css/themes/splide-default.min.css') }}">
    <link rel="stylesheet" href="{{ asset('js/plugins/splidejs/css/splide-extension-video.min.css') }}">
